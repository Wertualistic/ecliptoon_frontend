...
import { Library, Plus, Edit, Trash2, BookOpen, Upload, Tag, X, ChevronRight, AlertCircle, FileText } from 'lucide-react';
...
export default function AdminSeriesPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [seriesRes, genresRes, sponsorsRes] = await Promise.all([
        fetch(`${API_URL}/admin/series`, { headers: { 'Authorization': `Bearer ${token}` } }),
  ...
  useEffect(() => {
    if (chSelectedSeries) {
      const selected = seriesList.find(s => String(s.id) === chSelectedSeries);
      if (selected) {
  ...
  const handleDeleteSeries = async (id: number) => {
    if (!confirm('Ushbu seriyani o\'chirishni xohlaysizmi? Jami yuklangan rasmlar va boblar butunlay o\'chib ketadi.')) return;
    try {
  ...
  const handleChapterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const selected = seriesList.find(s => String(s.id) === chSelectedSeries);
        if (selected) {
          const chRes = await fetch(`${API_URL}/series/${selected.slug}/chapters`);
  ...
  const handleUploadImages = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_URL}/admin/chapters/${uploadingChId}/images`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/5 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Seriallar qo'shish, boblar yuklash, sahifalarni yangilash hamda janrlar ro'yxatini sozlash
          </p>
        </div>
          ...
          {showSeriesForm ? (
            ...
            <div className="glass-card p-6 rounded-2xl border border-violet-500/20 bg-violet-500/5 max-w-2xl">
              <h3 className="text-sm font-bold text-slate-200 uppercase tracking-wider mb-4 border-b border-white/5 pb-2">
                {editingSeries ? 'Serialni tahrirlash' : t('adminPanel.addSeries')}
              </h3>

              <form onSubmit={handleSeriesSubmit} className="space-y-4 text-xs text-slate-300">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-400">{t('adminPanel.titleField')}</label>
                    <input
                      type="text"
                      value={sTitle}
                  ...
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-400">Slug (URL uchun, ixtiyoriy)</label>
                    <input
                      type="text"
                      value={sSlug}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-400">{t('adminPanel.typeField')}</label>
                    <select
                      value={sType}
                  ...
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-400">{t('adminPanel.statusField')}</label>
                    <select
                      value={sStatus}
                    ...
                    >
                      ...
                      <option value="dropped">Tashlab ketilgan (Dropped)</option>
                    </select>
                  </div>
                </div>

                <div className="space-y-1">
                  <label className="font-semibold text-slate-400">{t('adminPanel.alternativeTitlesField')}</label>
                  <input
                    type="text"
                    value={sAltTitles}
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-slate-400">{t('adminPanel.descriptionField')}</label>
                  <textarea
                    value={sDescription}
                    onChange={(e) => setSDescription(e.target.value)}
                    rows={4}
                    className="w-full bg-slate-950 border border-white/10 text-slate-200 px-3 py-2 rounded-xl outline-none resize-none"
                  ></textarea>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="font-semibold text-slate-400">{t('adminPanel.coverImageField')}</label>
                    <input
                      type="file"
                      accept="image/*"
                ...
                {/* Genre checkboxes */}
                <div className="space-y-1">
                  <label className="font-semibold text-slate-400 block mb-1">{t('adminPanel.genresField')}</label>
                ...
                {/* Sponsor checkboxes */}
                <div className="space-y-1">
                  <label className="font-semibold text-slate-400 block mb-1">Homiy (Sponsor)</label>
                  <div className="flex flex-wrap gap-2 p-3 bg-slate-950 border border-white/5 rounded-xl max-h-32 overflow-y-auto">
                    {sponsors.length === 0 ? (
                      <span className="text-[10px] text-slate-500">Homiylar mavjud emas.</span>
                    ) : sponsors.map((sp) => (
                      <label 
                        key={sp.id} 
                        className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-semibold cursor-pointer transition-colors ${
                  ...
                  {seriesList.map((s) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <div>
                          <h4 className="font-bold text-xs text-slate-200 line-clamp-1">{s.title}</h4>
                          <span className="text-[10px] text-slate-500 uppercase font-semibold">{s.type} • {s.status}</span>
                        </div>
                      </div>
      ...
      activeTab === 'chapters' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 glass-card p-6 rounded-2xl border border-white/5 space-y-4">
            ...
            {/* Select Series */}
            <div className="space-y-1.5">
              <label className="text-xs text-slate-400 font-semibold">Serialni tanlang</label>
              <select
              ...
              <div className="space-y-2.5 pt-2">
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
                  {chaptersList.map((ch) => (
                    ...
                    >
                      <div>
                        <span className="font-bold text-slate-200">Bob {ch.chapter_number}</span>
                        {ch.title && <span className="text-slate-400 ml-2">- {ch.title}</span>}
                        <span className="text-[10px] text-slate-500 ml-3 font-semibold">
                          ({ch.is_free ? 'Bepul' : `${ch.price_in_diamonds} Olmos`})
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        {/* Page upload trigger */}
                        <button
                          onClick={() => { setUploadingChId(ch.id); setUploadSuccess(false); setError(null); }}
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            {showChapterForm && (
              <div className="glass-card p-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 space-y-4">
                ...
                <form onSubmit={handleChapterSubmit} className="space-y-3 text-xs text-slate-300">
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-400">{t('adminPanel.chapterNumberField')}</label>
                    <input
                      type="text"
                      placeholder="e.g. 1"
                      value={chNumber}
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    ...
                    {!chFree && (
                      <div className="space-y-1">
                        <label className="font-semibold text-slate-400">{t('adminPanel.priceField')}</label>
                        <input
                          type="number"
                          value={chPrice}
            ...
            {uploadingChId && (
              <div className="glass-card p-5 rounded-2xl border border-violet-500/20 bg-violet-500/5 space-y-4">
                <div className="flex justify-between items-start border-b border-violet-500/10 pb-2">
                  ...
                  <button onClick={() => setUploadingChId(null)} className="text-xs text-slate-400 hover:text-white">X</button>
                </div>
                  ...
                  <div className="border-2 border-dashed border-white/10 hover:border-violet-500/30 rounded-xl p-6 relative bg-slate-950/40 text-center">
                    ...
                    <div className="space-y-1 text-slate-400 text-xs">
                      ...
                      {uploadedPages && uploadedPages.length > 0 ? (
                        <p className="font-bold text-emerald-400">{uploadedPages[0].name}</p>
                      ) : (
